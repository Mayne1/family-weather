...
import InvitationDesignChooser from "./InvitationDesignChooser";
...
import { defaultInvitationStyle, normalizeInvitationStyle, recommendedInvitationStyle } from "../invitations/style";
...
export default function SavedInvitationEditor({ event, authorization }: { event: EventDetail; authorization: string }) {
  ...
  const [styleOptions, setStyleOptions] = useState(defaultInvitationStyle);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setStyleOptions(normalizeInvitationStyle(invitation.style_options));
