...
export default function EventDetailPage() {
  ...
  async function inviteMorePeople(formEvent: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEntitlement((current) => current && current.status !== "legacy" ? { ...current, email_consumed: current.email_consumed + newRows.length, email_remaining: Math.max(0, Number(current.email_remaining) - newRows.length) } : current);
