...
export default function HomePreview(){
 ...
 return <div className={styles.page}>
  ...
  <div className={styles.weatherTicker}><strong>RIGHT NOW</strong><span className={styles.tickerLocation}>{weather?.label||"Local weather"}</span><span><Icon name="sun"/><b>{read(current?.temp_f)}°</b> <i>{condition}</i></span><span>Feels like <b>{read(current?.feels_like_f)}°</b></span><span><Icon name="wind"/><b>{read(current?.wind_mph)} mph</b> wind</span><span><Icon name="rain"/><b>{read(day?.precip_prob_pct)}%</b> rain today</span><button type="button" onClick={useCurrentLocation}>Use my location</button><Link href="/today">Full forecast →</Link></div>
  <main>
