...
type Snapshot = { label?:string; current?:{temp_f:number;feels_like_f?:number;wind_mph:number;weather_code:number}|null; days?:WeatherDay[] };
...
export default function HomePreview(){
 ...
 const read=(v?:number)=>typeof v==="number"&&Number.isFinite(v)?Math.round(v):"—";
 ...
 return <div className={styles.page}>
  <header className={styles.header}><Link className={styles.brand} href="/"><span className={styles.brandOrb}><Icon name="sun"/></span><span><strong>Family</strong> Weather<small>PLAN THE DAY. SHARE THE OUTLOOK.</small></span></Link><nav><Link href="/today">Today</Link><Link href="/plan">Plan an activity</Link><Link href="/weather-history">Weather history</Link><Link href="/live">Live desk</Link><Link href="/weather-stories">Stories</Link><Link href="/events">My events</Link></nav><Link className={styles.signIn} href="/events">Sign in ↗</Link></header>
  <div className={styles.weatherTicker}><strong>RIGHT NOW</strong><span className={styles.tickerLocation}>{weather?.label||"Local weather"}</span><span><Icon name="sun"/><b>{read(current?.temp_f)}°</b> <i>{condition}</i></span><span><Icon name="wind"/><b>{read(current?.wind_mph)} mph</b> wind</span><span><Icon name="rain"/><b>{read(day?.precip_prob_pct)}%</b> rain</span><button type="button" onClick={useCurrentLocation}>Use my location</button><Link href="/today">Full forecast →</Link></div>
  <main>
