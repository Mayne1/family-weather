...
function scoreHour(hourly: HourlyCondition, profile: ActivityProfile, space: Space): ScoredHour {
  ...
  if (rain === null) {
    ...
    score -= 65;
    cautions.push("Rain is likely during this period.");
  } else if (rain > profile.maximumRainPct) {
    score -= Math.min(42, 12 + (rain - profile.maximumRainPct) * 0.8);
    cautions.push("Rain risk is higher than this activity tolerates well.");
  } else {
    reasons.push("Rain risk stays within a workable range.");
  }
