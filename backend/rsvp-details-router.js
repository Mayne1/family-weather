    ...
    try {
      ...
      const result = await pool.query(
        `SELECT i.token,i.invited_email,i.created_at,i.opened_at,i.responded_at,i.response,i.responder_name,i.guests_count,i.message,'invitation' AS source
         FROM invites i LEFT JOIN event_entitlements e ON e.event_id::text=i.event_id
         WHERE i.event_id=$1::text AND i.token<>COALESCE(e.share_invite_token,'')
         UNION ALL
         ...
         FROM share_link_rsvps s WHERE s.event_id=$1::bigint
